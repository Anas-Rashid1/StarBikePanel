...
const UserPage = () => {
  ...
  const fetchUsers = (pageNumber) => {
    axios
      .post("https://star-bike-backend.vercel.app/scooter/usertable", {
  ...
  return (
    <motion.div
      ...
      animate={{ scaleY: 1 }}
      exit={{ scaleY: 0 }}
      className="relative md:w-full h-screen overflow-x-hidden bg-gray-100 px-2"
    >
      <Search />
            ...
            <tbody>
              {users.map((item) => (
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                  ...
                  <td class="lg:px-2 md:px-4 px-2 text-sm ">{item.lastName}</td>
                  <td class="lg:px-2 md:px-4 px-2 text-sm">{item.email}</td>
                  <td
                    class={`lg:px-2 md:px-4 px-2  text-sm ${
                      item.status == true
                        ? "text-green-500"
                  ...
                  <td class="lg:px-2 md:px-2 px-4 ">{item.scooter}</td>
                </tr>
              ))}
              ...
              <tr colSpan={7} className="">
                <td colSpan={7} className="flex flex-row gap-2">
                  ...
                  {isMoreUsersAvailable ? (
                    <button
                      className="text-center text-orange-500 w-full"
                      onClick={handleReadMore}
                    >
                      Read More
                    </button>
                  ) : (
                    <p className="text-center text-orange-500">No More Users</p>
                  )}
                </td>
              </tr>
            </tbody>
